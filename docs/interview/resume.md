...
- 面向长任务中工具输出与历史消息持续膨胀的问题，设计四层 Context Engine：将超大工具结果外置为 Artifact，支持按关键词补取中部事实，并按水位执行陈旧清理、Microcompact 与高水位摘要；在 6 类上下文保真问答、3 轮 A/B 中将 Exact Match 从 `100%` 维持在 `100%`，累计 Prompt Token 从 `562,599` 降至 `146,022`（`-74.05%`）。
